...
const SoundProvider = ({ children }: SoundProviderProps) => {
    const [sounds, setSounds] = useState<{ id: string; path: string; repeat: boolean }[]>([]);

    const playSound = (soundName: string, repeat: boolean) => {
        const soundPath = soundPaths[soundName];
        if (soundPath) {
            setSounds((prevSounds) => [
                ...prevSounds.filter((sound) => sound.id !== soundName), // Remove existing sound with the same id
                { id: soundName, path: soundPath, repeat }
            ]);
        } else {
